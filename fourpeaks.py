...
def run_fourpeaks():
    ...
    if not os.path.exists(destination + '/FourPeaks/'):
        os.mkdir(destination + '/FourPeaks/')
    ...
    for prob_size in problem_size_space:
        ...
        fitness_four_peaks = mlrose.FourPeaks()
        generate_fourpeaks_state = lambda: np.random.randint(2, size=prob_size_int)
        init_state = generate_fourpeaks_state()
        problem = mlrose.DiscreteOpt(length=prob_size_int, fitness_fn=fitness_four_peaks, maximize=True, max_val=2)
        ...
        start = datetime.now()
        _, best_fitness_rhc, fit_array_rhc = mlrose.random_hill_climb(problem, max_attempts=200, max_iters=1000, curve=True, restarts=20)
        ...
        start = datetime.now()
        _, best_fitness_ga, fit_array_ga =  mlrose.genetic_alg(problem, pop_size=prob_size_int*5, curve=True, mutation_prob=.025, max_attempts=20, max_iters=1000)
    ...
    fit_frame.to_csv(destination + '/FourPeaks/problem_size_fit.csv')
    time_frame.to_csv(destination + '/FourPeaks/problem_size_time.csv')
    fit_iteration_frame.to_csv(destination + '/FourPeaks/fit_per_iteration.csv')
