...
def run_flipflop():
    ...
    if not os.path.exists(destination + '/FlipFlop/'):
        os.mkdir(destination + '/FlipFlop/')
    ...
    for prob_size in problem_size_space:
        ...
        generate_flip_flop_state = lambda: np.random.randint(2, size=prob_size_int)
        init_state = generate_flip_flop_state()
        problem = mlrose.DiscreteOpt(length=prob_size_int, fitness_fn=fitness_flip_flop, maximize=True, max_val=2)
    ...
    fit_frame.to_csv(destination + '/FlipFlop/problem_size_fit.csv')
    time_frame.to_csv(destination + '/FlipFlop/problem_size_time.csv')
    fit_iteration_frame.to_csv(destination + '/FlipFlop/fit_per_iteration.csv')
